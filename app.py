...
def on_create_game(data):
    ...
    try:
        ...
        max_players = 6
    code = gen_room_code()
    game = PokerGame(code, request.sid, name, max_players)
    GAMES[code] = game
...
def auto_play_loop():
    while True:
        ...
        for code in list(GAMES):
            game = GAMES.get(code)
            if not game or game.phase in ("lobby", "showdown"):
                continue
            ...
            socketio.emit(
                ...
                {
                    "name": "System",
                    "text": f"⏱️ {timeout_name} timed out — auto-{action}",
...
socketio.start_background_task(auto_play_loop)
