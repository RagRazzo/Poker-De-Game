...
class PokerGame:
    def __init__(self, room_code, host_sid, host_name):
    ...
    def mark_disconnected(self, sid):
        ...
        if (self.phase not in ("lobby", "showdown") and
                self.players[self.current_player_index]["sid"] == sid):
    ...
    def can_start(self, sid):
        return (
            sid == self.host_sid
            and 2 <= len(self.players) <= MAX_PLAYERS
    ...
    def start_round(self):
        ...
        if len(eligible) < 2:
            return False, "Not enough players with coins"
    ...
    def player_action(self, sid, action, amount=0):
        ...
        if action == "fold":
            ...
            # Allow all-in raise even below min
    ...
    def determine_winner(self):
        ...
        # Side-pot calculation based on total_bet levels
        ...
        for level in bet_levels:
            ...
            for i, (p, ev) in enumerate(winners):
                ...
                # Merge into existing result if player already won a side pot
